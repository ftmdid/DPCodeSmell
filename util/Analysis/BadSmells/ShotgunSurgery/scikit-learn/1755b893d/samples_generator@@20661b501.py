...
def make_multilabel_classification(n_samples=100, n_features=20, n_classes=5,
    ...
    if return_indicator in (True, 'sparse', 'dense'):
        ...
        Y = lb.fit([list(range(n_classes))]).transform(Y)
...
def make_sparse_coded_signal(n_samples, n_components, n_features,
    ...
    return list(map(np.squeeze, (Y, D, X)))
...
def make_biclusters(shape, n_clusters, noise=0.0, minval=10,
    ...
    row_labels = np.hstack(list(np.repeat(val, rep) for val, rep in
                                zip(list(range(n_clusters)), row_sizes)))
    col_labels = np.hstack(list(np.repeat(val, rep) for val, rep in
                                zip(list(range(n_clusters)), col_sizes)))
...
def make_checkerboard(shape, n_clusters, noise=0.0, minval=10,
    ...
    row_labels = np.hstack(list(np.repeat(val, rep) for val, rep in
                                zip(list(range(n_row_clusters)), row_sizes)))
    col_labels = np.hstack(list(np.repeat(val, rep) for val, rep in
                                zip(list(range(n_col_clusters)), col_sizes)))
