...
def test_structured_linkage_tree():
    ...
    for tree_builder in list(_TREE_BUILDERS.values()):
...
def test_height_linkage_tree():
    ...
    X = rng.randn(50, 100)
    connectivity = grid_to_graph(*mask.shape)
    for linkage_func in list(_TREE_BUILDERS.values()):
...
def test_agglomerative_clustering():
    ...
    for affinity in list(PAIRED_DISTANCES.keys()):
...
def test_scikit_vs_scipy():
    ...
    for linkage in list(_TREE_BUILDERS.keys()):
        for i in range(5):
