...
@pytest.mark.parametrize(
        'metric',
        chain(list(THRESHOLDED_METRICS.values()), list(REGRESSION_METRICS.values())))
...
@pytest.mark.parametrize('metric', list(CLASSIFICATION_METRICS.values()))
...
def test_multilabel_label_permutations_invariance(name):
    ...
    for perm in permutations(list(range(n_classes)), n_classes):
