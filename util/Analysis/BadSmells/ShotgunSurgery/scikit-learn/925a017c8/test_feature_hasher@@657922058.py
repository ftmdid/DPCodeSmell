...
def test_feature_hasher_dicts():
    ...
    gen = (iter(list(d.items())) for d in raw_X)
...
def test_feature_hasher_pairs():
    raw_X = (iter(list(d.items())) for d in [{"foo": 1, "bar": 2},
                                       {"baz": 3, "quux": 4, "foo": -1}])
...
def test_feature_hasher_pairs_with_string_values():
    raw_X = (iter(list(d.items())) for d in [{"foo": 1, "bar": "a"},
    ...
    raw_X = (iter(list(d.items())) for d in [{"bax": "abc"},
                                       {"bax": "abc"}])
    x1, x2 = h.transform(raw_X).toarray()
