...
def test_vectorizer_max_df():
    ...
    assert_equal(len(list(vect.vocabulary_.keys())), 5)

    vect.max_df = 0.5
    vect.fit(test_data)
    assert_true('a' not in list(vect.vocabulary_.keys()))  # 'a' is ignored
    assert_equal(len(list(vect.vocabulary_.keys())), 4)  # the others remain
...
def test_vectorizer_min_df():
    ...
    vect.min_df = 2
    vect.fit(test_data)
    assert_true('c' not in list(vect.vocabulary_.keys()))  # 'c' is ignored
    assert_equal(len(list(vect.vocabulary_.keys())), 2)  # only e, a remain
