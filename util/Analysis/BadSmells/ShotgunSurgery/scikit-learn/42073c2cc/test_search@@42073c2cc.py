...
def test_refit_callable():
    ...
    def refit_callable(cv_results):
        ...
        for key in list(clf.cv_results_.keys()):
...
def check_cv_results_array_types(search, param_keys, score_keys):
    ...
    scorer_keys = list(search.scorer_.keys()) if search.multimetric_ else ['score']
...
def compare_cv_results_multimetric_with_single(
    ...
    cv_results_acc_rec = {re.sub('_score$', '_accuracy', k): v
                          for k, v in list(search_acc.cv_results_.items())}
    cv_results_acc_rec.update({re.sub('_score$', '_recall', k): v
                               for k, v in list(search_rec.cv_results_.items())})
    ...
    np.testing.assert_equal({k: v for k, v in list(cv_results_multi.items())
                             if not k.endswith('_time')},
                            {k: v for k, v in list(cv_results_acc_rec.items())
                             if not k.endswith('_time')})
...
def test_grid_search_cv_splits_consistency():
    ...
    # setting.
    np.testing.assert_equal({k: v for k, v in list(gs.cv_results_.items())
                             if not k.endswith('_time')},
                            {k: v for k, v in list(gs2.cv_results_.items())
                             if not k.endswith('_time')})
