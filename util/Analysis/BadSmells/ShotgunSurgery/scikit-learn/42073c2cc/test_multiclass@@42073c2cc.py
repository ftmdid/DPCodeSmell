...
def test_unique_labels():
    ...
    # Multiclass problem
    assert_array_equal(unique_labels(list(range(10))), np.arange(10))
    ...
    assert_array_equal(unique_labels([4, 0, 2], list(range(5))),
                       np.arange(5))
...
def test_is_multilabel():
    for group, group_examples in list(EXAMPLES.items()):
...
def test_check_classification_targets():
    for y_type in list(EXAMPLES.keys()):
