...
def pairwise_distances_argmin_min(X, Y, axis=1, metric="euclidean",
    ...
    indices, values = list(zip(*pairwise_distances_chunked(
...
def _pairwise_callable(X, Y, metric, **kwds):
    ...
    if X is Y:
        ...
        iterator = itertools.combinations(list(range(X.shape[0])), 2)
        for i, j in iterator:
        ...
        iterator = itertools.product(list(range(X.shape[0])), list(range(Y.shape[0])))
        for i, j in iterator:
            out[i, j] = metric(X[i], Y[j], **kwds)
