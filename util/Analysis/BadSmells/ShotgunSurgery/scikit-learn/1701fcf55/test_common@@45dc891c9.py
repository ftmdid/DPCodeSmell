...
def test_sample_order_invariance():
    ...
    for name, metric in list(ALL_METRICS.items()):
...
def test_invariance_string_vs_numbers_labels():
    ...
    for name, metric in list(CLASSIFICATION_METRICS.items()):
        if name in METRIC_UNDEFINED_BINARY_MULTICLASS:
            continue
    ...
    for name, metric in list(THRESHOLDED_METRICS.items()):
...
def test_inf_nan_input():
    ...
    for metric in list(CLASSIFICATION_METRICS.values()):
        for y_true, y_score in invalids:
            assert_raise_message(ValueError,
