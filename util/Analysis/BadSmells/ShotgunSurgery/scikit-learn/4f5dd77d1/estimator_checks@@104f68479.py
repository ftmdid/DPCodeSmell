...
def _yield_classifier_checks(name, classifier):
    ...
    if 'class_weight' in list(classifier.get_params().keys()):
...
def check_dont_overwrite_parameters(name, estimator_orig):
    ...
    public_keys_after_fit = [key for key in list(dict_after_fit.keys())
                             if is_public_parameter(key)]

    attrs_added_by_fit = [key for key in public_keys_after_fit
                          if key not in list(dict_before_fit.keys())]
...
def _apply_on_subsets(func, X):
    ...
    if type(result_full) == tuple:
        ...
        result_by_batch = list([x[0] for x in result_by_batch])
...
def check_fit_score_takes_y(name, estimator_orig):
    ...
    for func_name in funcs:
        ...
        if func is not None:
            func(X, y)
            args = [p.name for p in list(signature(func).parameters.values())]
            if args[0] == "self":
...
def check_estimators_overwrite_params(name, estimator_orig):
    ...
    for param_name, original_value in list(original_params.items()):
...
def check_parameters_default_constructible(name, Estimator):
    ...
    with ignore_warnings(category=(DeprecationWarning, FutureWarning)):
        ...
        try:
            ...
            init_params = [p for p in list(signature(init).parameters.values())
                           if param_filter(p)]
        ...
        for init_param in init_params:
            ...
            if init_param.name not in list(params.keys()):
...
def check_get_params_invariance(name, estimator_orig):
    ...
    assert_true(all(item in list(deep_params.items()) for item in
                    list(shallow_params.items())))
...
def check_set_params(name, estimator_orig):
    ...
    for k, v in list(curr_params.items()):
    ...
    for param_name in list(orig_params.keys()):
...
def check_fit_idempotent(name, estimator_orig):
    ...
    estimator = clone(estimator_orig)
    set_random_state(estimator)
    if 'warm_start' in list(estimator.get_params().keys()):
