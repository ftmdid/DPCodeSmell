...
def all_estimators(include_meta_estimators=False,
    ...
    if type_filter is not None:
        ...
        for name, mixin in list(filters.items()):
...
def _get_args(function, varargs=False):
    ...
    try:
        ...
        return []
    args = [key for key, param in list(params.items())
            if param.kind not in (param.VAR_POSITIONAL, param.VAR_KEYWORD)]
    if varargs:
        varargs = [param.name for param in list(params.values())
...
def _get_func_name(func, class_name=None):
    ...
    if class_name is not None:
        ...
        parts.append(func.__self__.__class__.__name__)
...
def check_docstring_parameters(func, doc=None, ignore=None, class_name=None):
    ...
    if func_name.split('.')[2] == 'estimator_checks':
        return incorrect
    args = list([x for x in _get_args(func) if x not in ignore])
    # drop self
    ...
    param_names = list([x for x in param_names if x not in ignore])
