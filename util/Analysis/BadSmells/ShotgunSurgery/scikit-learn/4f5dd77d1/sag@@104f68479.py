...
def sag_solver(X, y, sample_weight=None, loss='log', alpha=1., beta=0.,
    ...
    if 'coef' in list(warm_start_mem.keys()):
    ...
    if 'intercept_sum_gradient' in list(warm_start_mem.keys()):
        intercept_sum_gradient = warm_start_mem['intercept_sum_gradient']
    else:
        intercept_sum_gradient = np.zeros(n_classes, dtype=np.float64)

    if 'gradient_memory' in list(warm_start_mem.keys()):
        gradient_memory_init = warm_start_mem['gradient_memory']
    else:
        gradient_memory_init = np.zeros((n_samples, n_classes),
                                        dtype=np.float64, order='C')
    if 'sum_gradient' in list(warm_start_mem.keys()):
        sum_gradient_init = warm_start_mem['sum_gradient']
    else:
        sum_gradient_init = np.zeros((n_features, n_classes),
                                     dtype=np.float64, order='C')

    if 'seen' in list(warm_start_mem.keys()):
        seen_init = warm_start_mem['seen']
    else:
        seen_init = np.zeros(n_samples, dtype=np.int32, order='C')

    if 'num_seen' in list(warm_start_mem.keys()):
