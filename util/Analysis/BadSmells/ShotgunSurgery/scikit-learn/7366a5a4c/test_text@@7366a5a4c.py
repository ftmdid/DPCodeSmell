...
def test_vectorizer_max_df():
    ...
    vect.fit(test_data)
    assert 'a' in list(vect.vocabulary_.keys())
    assert len(list(vect.vocabulary_.keys())) == 6
    assert len(vect.stop_words_) == 0

    vect.max_df = 0.5  # 0.5 * 3 documents -> max_doc_count == 1.5
    vect.fit(test_data)
    assert 'a' not in list(vect.vocabulary_.keys())  # {ae} ignored
    assert len(list(vect.vocabulary_.keys())) == 4    # {bcdt} remain
...
def test_vectorizer_min_df():
    ...
    vect.min_df = 2
    vect.fit(test_data)
    assert 'c' not in list(vect.vocabulary_.keys())  # {bcdt} ignored
    assert len(list(vect.vocabulary_.keys())) == 2    # {ae} remain
    assert 'c' in vect.stop_words_
    assert len(vect.stop_words_) == 4

    vect.min_df = 0.8  # 0.8 * 3 documents -> min_doc_count == 2.4
    vect.fit(test_data)
    assert 'c' not in list(vect.vocabulary_.keys())  # {bcdet} ignored
    assert len(list(vect.vocabulary_.keys())) == 1    # {a} remains
    assert 'c' in vect.stop_words_
