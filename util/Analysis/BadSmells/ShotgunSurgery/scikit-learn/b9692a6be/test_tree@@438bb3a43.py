...
def test_classification_toy():
    ...
    for name, Tree in list(CLF_TREES.items()):
...
def test_regression_toy():
    ...
    for name, Tree in list(REG_TREES.items()):
...
def test_iris():
    ...
    for (name, Tree), criterion in product(list(CLF_TREES.items()), CLF_CRITERIONS):
...
def test_boston():
    ...
    for (name, Tree), criterion in product(list(REG_TREES.items()), REG_CRITERIONS):
...
def test_pure_set():
    ...
    for name, TreeClassifier in list(CLF_TREES.items()):
        clf = TreeClassifier(random_state=0)
        clf.fit(X, y)
        assert_array_equal(clf.predict(X), y,
                           err_msg="Failed with {0}".format(name))

    for name, TreeRegressor in list(REG_TREES.items()):
...
def test_max_features():
    ...
    for name, TreeEstimator in list(ALL_TREES.items()):
...
def test_error():
    ...
    for name, TreeEstimator in list(CLF_TREES.items()):
        # predict before fit
        est = TreeEstimator()
...
def test_min_samples_split():
    ...
    for max_leaf_nodes, name in product((None, 1000), list(ALL_TREES.keys())):
...
def test_memory_layout():
    ...
    for (name, TreeEstimator), dtype in product(list(ALL_TREES.items()),
...
def test_with_only_one_non_constant_features():
    ...
    for name, TreeEstimator in list(REG_TREES.items()):
        est = TreeEstimator(random_state=0, max_features=1)
        est.fit(X, y)
        assert est.tree_.max_depth == 1
...
def test_criterion_copy():
    ...
    for copy_func in [copy.copy, copy.deepcopy, _pickle_copy]:
        for _, typename in list(CRITERIA_CLF.items()):
        ...
        for _, typename in list(CRITERIA_REG.items()):
