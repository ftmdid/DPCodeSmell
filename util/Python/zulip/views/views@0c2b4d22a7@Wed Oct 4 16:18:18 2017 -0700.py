...
def sort_by_totals(value_arrays):
    ...
    totals = [(sum(values), label) for label, values in list(value_arrays.items())]
...
def sort_client_labels(data):
    ...
    return [label for label, sort_value in sorted(list(label_sort_values.items()),
                                                  key=lambda x: x[1])]
...
def rewrite_client_arrays(value_arrays):
    ...
    for label, array in list(value_arrays.items()):
...
def get_time_series_by_subgroup(stat, table, key_id, end_times, subgroup_to_label, include_empty_subgroups):
    ...
    value_arrays = {}
    for subgroup, label in list(subgroup_to_label.items()):
