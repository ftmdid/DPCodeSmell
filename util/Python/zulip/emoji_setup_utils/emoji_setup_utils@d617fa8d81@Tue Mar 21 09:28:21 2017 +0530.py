...
def emoji_names_for_picker(emoji_map):
    ...
    for name, codepoint in list(emoji_map.items()):
    ...
    for func in [blacklisted, one_lettered, ideographless, word_superset,
        ...
        for codepoint, names in list(codepoint_to_names.items()):
    ...
    return sorted(list(chain.from_iterable(list(codepoint_to_names.values()))))
