...
def ticket_started_body(payload: Dict[str, Any]) -> Text:
    body = 'New ticket from {customer_name}'
...
def ticket_assigned_body(payload: Dict[str, Any]) -> Text:
    ...
    if state == "opened":
        body = "An open ticket has been assigned to"
    else:
        body = "A {state} ticket has been assigned to"
    ...
    if assignee or assigned_group:
        if assignee and assigned_group:
            body += " {assignee} from {assigned_group}"
        elif assignee:
            body += " {assignee}"
        elif assigned_group:
            body += " {assigned_group}"
...
def agent_replied_body(payload: Dict[str, Any]) -> Text:
    ...
    body = "%s has just replied to a ticket\n```quote\n**[Ticket #%s]" % (agent, number)
    body += "({app_ticket_url})**\n{plain_text_body}\n```"
    return body.format(**payload)
