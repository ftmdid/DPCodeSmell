...
def move_messages_to_archive(message_ids: List[int]) -> None:
    ...
    user_messages = list(UserMessage.objects.filter(
    ...
    for model in models_with_message_key:
        elements = list(model['class'].objects.filter(message_id__in=message_ids).exclude(
    ...
    attachments = list(Attachment.objects.filter(messages__id__in=message_ids).exclude(
