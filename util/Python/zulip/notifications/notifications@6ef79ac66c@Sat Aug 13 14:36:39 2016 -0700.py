...
def pm_narrow_url(participants):
    ...
    participants.sort()
    base_url = "%s/#narrow/pm-with/" % (settings.SERVER_URI,)
    return base_url + hashchange_encode(",".join(participants))

def stream_narrow_url(stream):
    # type: (text_type) -> text_type
    base_url = "%s/#narrow/stream/" % (settings.SERVER_URI,)
...
def handle_missedmessage_emails(user_profile_id, missed_email_events):
    ...
    message_count_by_recipient_subject = {
        ...
        for recipient_subject, msgs in list(messages_by_recipient_subject.items())
    }

    for msg_list in list(messages_by_recipient_subject.values()):
    ...
    for recipient_subject, msg_list in list(messages_by_recipient_subject.items()):
