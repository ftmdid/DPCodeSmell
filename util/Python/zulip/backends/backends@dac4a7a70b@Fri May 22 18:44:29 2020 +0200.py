...
class ZulipLDAPAuthBackendBase(ZulipAuthMixin, LDAPBackend):
    ...
    def sync_custom_profile_fields_from_ldap(self, user_profile: UserProfile,
        ...
        for attr, ldap_attr in list(settings.AUTH_LDAP_USER_ATTR_MAP.items()):
        ...
        for var_name, value in list(values_by_var_name.items()):
            try:
...
def query_ldap(email: str) -> List[str]:
    ...
    if backend is not None:
        ...
        for django_field, ldap_field in list(settings.AUTH_LDAP_USER_ATTR_MAP.items()):
...
class SAMLAuthBackend(SocialAuthMixin, SAMLAuth):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        if settings.SAML_REQUIRE_LIMIT_TO_SUBDOMAINS:
            idps_without_limit_to_subdomains = [
                idp_name for idp_name, idp_dict in list(settings.SOCIAL_AUTH_SAML_ENABLED_IDPS.items())
    ...
    def get_issuing_idp(cls, SAMLResponse: str) -> Optional[str]:
        ...
        for idp_name, idp_config in list(settings.SOCIAL_AUTH_SAML_ENABLED_IDPS.items()):
    ...
    def auth_complete(self, *args: Any, **kwargs: Any) -> Optional[HttpResponse]:
        ...
        result = None
        try:
            for param, value in list(relayed_params.items()):
    ...
    def dict_representation(cls, realm: Optional[Realm]=None) -> List[ExternalAuthMethodDictT]:
        ...
        for idp_name, idp_dict in list(settings.SOCIAL_AUTH_SAML_ENABLED_IDPS.items()):
