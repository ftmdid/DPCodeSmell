...
class Realm(models.Model):
    ...
    def get_admin_users(self):
        ...
        return list(filter(is_admin, candidates))
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    def __repr__(self):
        return ("<UserProfile: %s %s>" % (self.email, self.realm)).encode("utf-8")
    def __str__(self):
        return self.__repr__()
...
class UserMessage(models.Model):
    ...
    def __repr__(self):
        ...
        return ("<UserMessage: %s / %s (%s)>" % (display_recipient, self.user_profile.email, self.flags_list())).encode("utf-8")
