...
class Realm(ModelReprMixin, models.Model):
    ...
    AUTHENTICATION_FLAGS = ['Google', 'Email', 'GitHub', 'LDAP', 'Dev', 'RemoteUser']
    ...
    default_language = models.CharField(default='en', max_length=MAX_LANGUAGE_ID_LENGTH) # type: Text
    ...
    def authentication_methods_dict(self):
        ...
        for k, v in self.authentication_methods.items():
...
class RealmEmoji(ModelReprMixin, models.Model):
    ...
    def __unicode__(self):
        # type: () -> Text
        return "<RealmEmoji(%s): %s %s>" % (self.realm.string_id, self.name, self.img_url)
...
def get_realm_filters_cache_key(realm_id):
    # type: (int) -> Text
    return 'all_realm_filters:%s' % (realm_id,)
...
class UserProfile(ModelReprMixin, AbstractBaseUser, PermissionsMixin):
    ...
    rate_limits = models.CharField(default="", max_length=100) # type: Text # comma-separated list of range:max pairs
    ...
    # completed.
    onboarding_steps = models.TextField(default='[]') # type: Text

    invites_granted = models.IntegerField(default=0) # type: int
    invites_used = models.IntegerField(default=0) # type: int

    alert_words = models.TextField(default='[]') # type: Text # json-serialized list of strings

    # Contains serialized JSON of the form:
    # [["social", "mit"], ["devel", "ios"]]
    muted_topics = models.TextField(default='[]') # type: Text
    ...
    timezone = models.CharField(max_length=40, default='')  # type: Text
    ...
    def profile_data(self):
        ...
        for field in custom_profile_fields_for_realm(self.realm_id):
            ...
            for k, v in list(field.as_dict().items()):
                field_data[k] = v
...
class Stream(ModelReprMixin, models.Model):
    ...
    description = models.CharField(max_length=1024, default='') # type: Text
...
class AbstractUserMessage(ModelReprMixin, models.Model):
    ...
    def flags_list(self):
        # type: () -> List[str]
        return [flag for flag in list(self.flags.keys()) if getattr(self.flags, flag).is_set]
...
class UserMessage(AbstractUserMessage):
    ...
    def __unicode__(self):
        ...
        return "<UserMessage: %s / %s (%s)>" % (display_recipient, self.user_profile.email, self.flags_list())
...
class CustomProfileField(models.Model):
    ...
    FIELD_TYPE_DATA = [
        ...
        (INTEGER, 'Integer', check_int, int),
        (FLOAT, 'Float', check_float, float),
        (SHORT_TEXT, 'Short Text', check_short_string, str),
        (LONG_TEXT, 'Long Text', check_string, str),
