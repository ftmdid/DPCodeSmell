...
def has_valid_realm(value):
    try:
        ...
        raise ValidationError(mark_safe('Registration is not currently available for your domain. ' + SIGNUP_STRING))

def isnt_mit(value):
    if "@mit.edu" in value:
        raise ValidationError(mark_safe('Humbug for MIT is by invitation only. ' + SIGNUP_STRING))
