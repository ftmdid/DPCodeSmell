...
def do_update_message(user_profile, message_id, subject, propagate_mode, content):
    ...
    notice = dict(event=event, users=list(map(user_info, ums)), type='update_message')
    tornado_callbacks.send_notification(notice)
...
def decode_email_address(email):
    ...
    token = match.group(1)
    decoded_token = re.sub("%\d{4}", lambda x: chr(int(x.group(0)[1:])), token)
    ...
    for event in events:
        if event['type'] == "message":
            ...
            if event['op'] == "remove" or event['op'] == "update":
                person = event['person']
                state['realm_users'] = [p for p in state['realm_users'] if p['email'] != person['email']]
            ...
            if event['op'] == "add":
                ...
                state['unsubscribed'] = [s for s in state['unsubscribed'] if s['name'].lower() not in subscriptions_to_filter]
            elif event['op'] == "remove":
                state['unsubscribed'] += event['subscriptions']
                state['subscriptions'] = [s for s in state['subscriptions'] if s['name'].lower() not in subscriptions_to_filter]
...
def send_future_email(recipients, email_html, email_text, subject,
    ...
    if delay < datetime.timedelta(minutes=1):
        results = mail_client.messages.send(message=message, asyncl=False, ip_pool="Main Pool")
    else:
        send_time = (datetime.datetime.utcnow() + delay).__format__("%Y-%m-%d %H:%M:%S")
        results = mail_client.messages.send(message=message, asyncl=False, ip_pool="Main Pool", send_at=send_time)
