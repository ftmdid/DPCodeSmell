...
def send_future_email(recipients, email_html, email_text, subject,
    ...
    if delay < datetime.timedelta(minutes=1):
        results = mail_client.messages.send(message=message, asynck=False, ip_pool="Main Pool")
    else:
        send_time = (datetime.datetime.utcnow() + delay).__format__("%Y-%m-%d %H:%M:%S")
        results = mail_client.messages.send(message=message, asynck=False, ip_pool="Main Pool", send_at=send_time)
