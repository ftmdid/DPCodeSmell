...
def generate_topic_history_from_db_rows(rows: List[Tuple[str, int]]) -> List[Dict[str, Any]]:
    ...
    history = []
    for canonical_topic, (max_message_id, topic_name) in list(canonical_topic_names.items()):
        history.append(dict(
            name=topic_name,
...
def do_send_messages(messages_maybe_none: Sequence[Optional[MutableMapping[str, Any]]],
    ...
    for message in messages:
        ...
        for queue_name, events in list(message['message'].service_queue_events.items()):
            for event in events:
                queue_json_publish(
                    queue_name,
                    {
...
def bulk_add_subscriptions(streams: Iterable[Stream],
    ...
    recipients = [recipient.id for recipient in list(recipients_map.values())]  # type: List[int]
...
def bulk_remove_subscriptions(users: Iterable[UserProfile],
    ...
    # bulk operations.
    for tup_list in list(existing_subs_by_user.values()):
...
def set_default_streams(realm: Realm, stream_dict: Dict[str, Dict[str, Any]]) -> None:
    ...
    stream_names = []
    for name, options in list(stream_dict.items()):
...
def get_cross_realm_dicts() -> List[Dict[str, Any]]:
    users = list(bulk_get_users(list(settings.CROSS_REALM_BOT_EMAILS), None,
...
def get_service_dicts_for_bots(bot_dicts: List[Dict[str, Any]],
    ...
    for bot_dict in bot_dicts:
        ...
        if bot_type  == UserProfile.OUTGOING_WEBHOOK_BOT:
            ...
            if bot_profile_id in list(embedded_bot_configs.keys()):
