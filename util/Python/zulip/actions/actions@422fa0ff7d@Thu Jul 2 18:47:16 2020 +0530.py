...
def do_set_realm_message_editing(realm: Realm,
    ...
    for updated_property, updated_value in list(updated_properties.items()):
...
def bulk_add_subscriptions(streams: Iterable[Stream],
    ...
    recipient_ids: List[int] = [recipient_id for recipient_id in list(recipients_map.values())]
...
def do_clear_mobile_push_notifications_for_ids(user_profile_ids: List[int],
    ...
    for (user_profile_id, event_message_ids) in list(messages_by_user.items()):
...
def update_to_dict_cache(changed_messages: List[Message], realm_id: Optional[int]=None) -> List[int]:
    ...
    for msg_id, msg in list(changed_messages_to_dict.items()):
