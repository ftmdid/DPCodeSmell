...
def fetch_fullname(username):
    try:
        ...
        print('%s: zephyr=>humbug: Error getting fullname for %s' % \
    ...
    while True:
        ...
        if notice is not None:
            try:
                ...
                print('%s: zephyr=>humbug: Error relaying zephyr' % \
...
def process_notice(notice, log):
    ...
    if isinstance(zsig, str):
        ...
        if '\u200B'.encode("utf-8") in zsig or zsig.endswith("@(@color(blue))"):
...
def forward_to_zephyr(message):
    zsig = "%s@(@color(blue))" % (username_to_fullname(message["sender_email"]))
    if ' dot ' in zsig:
...
def parse_zephyr_subs(verbose=False):
    ...
    if not os.path.exists(subs_file):
        if verbose:
            print("Couldn't find ~/.zephyr.subs!", file=sys.stderr)
        return []
    ...
    for line in file(subs_file, "r").readlines():
        ...
        try:
            ...
            if not valid_stream_name(cls):
                if verbose:
                    print("Skipping subscription to unsupported class name: [%s]" % (line,), file=sys.stderr)
                continue
        except:
            if verbose:
                print("Couldn't parse ~/.zephyr.subs line: [%s]" % (line,), file=sys.stderr)
            continue
