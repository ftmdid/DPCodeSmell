...
def get_push_event_body(payload):
    ...
    body = "{} pushed [{} commit{}]({}) to {} branch.".format(
        get_user_name(payload),
        total_commits_count,
        's' if total_commits_count > 1 else '',
        compare_url,
...
def get_issue_created_event_body(payload):
    ...
    if assignee_name:
        body = "{} (assigned to {}).".format(body[:-1], assignee_name)
    return body
...
def get_commented_commit_event_body(payload):
    # type: (Dict[str, Any]) -> text_type
    return "{} added [comment]({}) to [Commit]({}).".format(
        get_issue_user_name(payload),
...
def get_commented_merge_request_event_body(payload):
    # type: (Dict[str, Any]) -> text_type
    return "{} added [comment]({}) to [Merge Request #{}]({}/merge_requests/{}).".format(
        get_issue_user_name(payload),
        payload.get('object_attributes').get('url'),
...
def get_commented_issue_event_body(payload):
    # type: (Dict[str, Any]) -> text_type
    return "{} added [comment]({}) to [Issue #{}]({}/issues/{}).".format(
        get_issue_user_name(payload),
        payload.get('object_attributes').get('url'),
...
def get_commented_snippet_event_body(payload):
    # type: (Dict[str, Any]) -> text_type
    return "{} added [comment]({}) to [Snippet #{}]({}/snippets/{}).".format(
        get_issue_user_name(payload),
        payload.get('object_attributes').get('url'),
...
def get_subject_based_on_event(event, payload):
    ...
    if event == 'Push Hook':
        ...
        return "{} / {}".format(payload.get('repository').get('name'), get_branch_name(payload))
