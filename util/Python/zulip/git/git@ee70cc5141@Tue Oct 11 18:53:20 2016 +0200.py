...
SUBJECT_WITH_PR_INFO_TEMPLATE = '{repo} / {type} #{id} {title}'

EMPTY_SHA = '0000000000000000000000000000000000000000'

PUSH_COMMITS_LIMIT = 10
PUSH_PUSHED_TEXT_WITH_URL = "[pushed]({compare_url})"
PUSH_PUSHED_TEXT_WITHOUT_URL = "pushed"
PUSH_COMMIT_ROW_TEMPLATE = '* [{commit_short_sha}]({commit_url}): {commit_msg}\n'
PUSH_COMMITS_MORE_THAN_LIMIT_TEMPLATE = "[and {commits_number} more commit(s)]"
PUSH_COMMITS_MESSAGE_TEMPLATE = """{user_name} {pushed_text} to branch {branch_name}
...
PULL_REQUEST_MESSAGE_TEMPLATE = "{user_name} {action} [{type}]({url})"
PULL_REQUEST_ASSIGNEE_INFO_TEMPLATE = "(assigned to {assignee})"
PULL_REQUEST_BRANCH_INFO_TEMPLATE = "\nfrom `{target}` to `{base}`"
PULL_REQUEST_CONTENT_MESSAGE_TEMPLATE = "\n~~~ quote\n{message}\n~~~"
