...
def send_android_push_notification(devices: List[DeviceToken], data: Dict[str, Any],
    ...
    if res and 'success' in res:
        for reg_id, msg_id in list(res['success'].items()):
    ...
    if 'canonical' in res:
        for reg_id, new_reg_id in list(res['canonical'].items()):
    ...
    if 'errors' in res:
        for error, reg_ids in list(res['errors'].items()):
...
def get_mobile_push_content(rendered_content: str) -> str:
    ...
    def format_as_quote(quote_text: str) -> str:
        quote_text_list = [_f for _f in quote_text.split('\n') if _f]  # Remove empty lines
        quote_text = '\n'.join(["> "+x for x in quote_text_list])
        quote_text += '\n'
        return quote_text
