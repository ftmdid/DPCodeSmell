...
def find_openapi_endpoint(path: str) -> Optional[str]:
    for path_regex, endpoint in list(openapi_spec.endpoints_dict().items()):
...
def validate_against_openapi_schema(content: Dict[str, Any], path: str,
    ...
    if status_code.startswith('5'):
        return False
    if path not in list(openapi_spec.openapi()['paths'].keys()):
...
def validate_schema(schema: Dict[str, Any]) -> None:
    ...
    for property_schema in list(schema.get('properties', dict()).values()):
