...
def handle_missedmessage_emails(user_profile_id: int,
    ...
    message_count_by_bucket = {
        bucket_tup: len(msgs)
        for bucket_tup, msgs in list(messages_by_bucket.items())
    }

    for msg_list in list(messages_by_bucket.values()):
    ...
    for bucket_tup, msg_list in list(messages_by_bucket.items()):
