...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        users_who_need_colors = [profile for profile in user_profiles if Subscription.objects.filter(
                user_profile=profile).filter(~Q(color=Subscription.DEFAULT_STREAM_COLOR)).count() == 0]
