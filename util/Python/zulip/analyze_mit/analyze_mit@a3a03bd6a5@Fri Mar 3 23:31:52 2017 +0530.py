...
def compute_stats(log_level):
    ...
    for email, counts in list(user_counts.items()):
        total_user_counts.setdefault(email, 0)
        for client_name, count in list(counts.items()):
    ...
    for size in [10, 25, 50, 100, 200, len(list(total_user_counts.keys()))]:
        top_percents[size] = 0.0
    for i, email in enumerate(sorted(list(total_user_counts.keys()),
        ...
        for size in list(top_percents.keys()):
    ...
    for client in list(total_counts.keys()):
