...
strip_whitespace_left = re.compile("\\s+(%s-\\s*(endtrans|pluralize).*?-?%s)" % (BLOCK_TAG_START, BLOCK_TAG_END), re.U)
...
class Command(makemessages.Command):
    ...
    def get_new_strings(self, old_strings, translation_strings):
        ...
        plurals = {k: v for k, v in list(old_strings.items()) if k.endswith('_plural')}
        for plural_key, value in list(plurals.items()):
