...
def check_realm_emoji_update(var_name: str, event: Dict[str, object]) -> None:
    ...
    for k, v in list(event["realm_emoji"].items()):
        assert v["id"] == k
...
def check_realm_update(var_name: str, event: Dict[str, object], prop: str,) -> None:
    ...
    if prop == "plan_type":
        ...
        assert "extra_data" in list(event.keys())
        return

    assert "extra_data" not in list(event.keys())
...
def check_subscription_add(
    ...
    for sub in event["subscriptions"]:
        if include_subscribers:
            assert "subscribers" in list(sub.keys())
        else:
            assert "subscribers" not in list(sub.keys())
...
def check_update_display_settings(var_name: str, event: Dict[str, object],) -> None:
    ...
    if setting_name == "default_language":
        assert "language_name" in list(event.keys())
    else:
        assert "language_name" not in list(event.keys())
