...
def _cholesky_omp(X, y, n_nonzero_features, eps=None):
    """
    ...
    eps: float
        Targeted squared error, if not None overrides n_nonzero_features.

    Returns:
    --------
    gamma: array, shape n_nonzero_features
        Non-zero elements of the solution

    idx: array, shape n_nonzero_features
    ...
    for it in xrange(X.shape[1]):
        ...
        if eps != None and np.dot(residual.T, residual) <= eps:
            break
        elif n_nonzero_features != None and it == n_nonzero_features - 1:
            break

    return gamma, idx


def _gram_omp(G, Xy, n_nonzero_features, eps_0=None, eps=None):
    """
    Solves a single Orthogonal Matching Pursuit problem using
...
def orthogonal_mp(X, y, n_nonzero_features=None, eps=None, compute_gram=False):
    """Orthogonal Matching Pursuit (OMP)

    Solves n_targets Orthogonal Matching Pursuit problems.
    An instance of the problem has the form:

    When parametrized by number of non-zero elements using `n_nonzero_features`:
    argmin ||y - X\gamma||^2 subject to ||\gamma||_0 <= n_{nonzero features}
    ...
    eps: float
        Maximum norm of the residual. If not None, overrides n_nonzero_features.
    ...
    Returns
    -------
    coef: array of shape: n_nonzero_features or (n_nonzero_features, n_targets)
    ...
    if y.ndim == 1:
        y = y[:, np.newaxis]
    if n_nonzero_features == None and eps == None:
        raise ValueError('OMP needs either a target number of atoms (n_nonzero_features) \
                         or a target residual error (eps)')
    if eps != None and eps < 0:
        raise ValueError("Epsilon cannot be negative")
    if eps == None and n_nonzero_features <= 0:
        raise ValueError("The number of atoms must be positive")
    if eps == None and n_nonzero_features > X.shape[1]:
    ...
    if compute_gram:
        ...
        if eps != None:
            ...
            norms_squared = None
        return orthogonal_mp_gram(G, Xy, n_nonzero_features, eps, norms_squared)

    coef = np.zeros((X.shape[1], y.shape[1]))
    for k in xrange(y.shape[1]):
        x, idx = _cholesky_omp(X, y[:, k], n_nonzero_features, eps)
        coef[idx, k] = x
    return np.squeeze(coef)


def orthogonal_mp_gram(G, Xy, n_nonzero_features=None, eps=None, norms_squared=None):
    ...
    if n_nonzero_features == None and eps == None:
        raise ValueError('OMP needs either a target number of atoms \
                         (n_nonzero_features) or a target residual error (eps)')
    ...
    if eps == None and n_nonzero_features <= 0:
        raise ValueError("The number of atoms must be positive")
    if eps == None and n_nonzero_features > len(G):
        raise ValueError("The number of atoms cannot be more than the number \
                          of features")
    coef = np.zeros((len(G), Xy.shape[1]))
    for k in range(Xy.shape[1]):
        x, idx = _gram_omp(G, Xy[:, k], n_nonzero_features,
